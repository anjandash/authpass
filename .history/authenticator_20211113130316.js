...
xxxnew_button.addEventListener("click", async () => {
        ...
        for (let device of devices) {
            ...
            try{
                ...
                device.addEventListener('advertisementreceived', evt => {
                    //abortController.abort();
                    device.gatt.connect().then(gattServer => resolve(gattServer)).catch(error => reject(error));     
                });           
            } catch (err) {
                console.log(err);
...
function onWatchAdvertisementsButtonClick() {
    ...
    navigator.bluetooth.getDevices().then(device => {
